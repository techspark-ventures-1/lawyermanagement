...
function lawyermanagement()
{
	...
	// Perform SQL query to fetch case records
	$query = "SELECT case_name, case_number, case_type FROM cases";
...
// Close the database connection when you're done fetching records
// $mysqli->close();
?>
		...
		<div id="page-wrapper">
			...
			<div class="existing-records">
				<div style="text-align: right; margin-bottom: 10px;">
					...
					<a href="addclient.php" class="btn btn-primary pull-right">Add Client</a>
				</div>
				...
				<table>
					...
					<tbody>
						...
						<?php
						// Example PHP code to fetch and display records from the database
						...
						// Perform SQL query to fetch case records
						$query = "SELECT f_name, l_name, email, mobile FROM lclient  WHERE lawyer_id = '" . $_SESSION["lawyer_id"]. "'";
						...
						if ($records) {
							foreach ($records as $record) {
								echo "<tr>";
								// Assuming you have columns named 'f_name' and 'l_name' in your database
								echo "<td>" . (isset($record['f_name']) ? $record['f_name'] : '') . " " . (isset($record['l_name']) ? $record['l_name'] : '') . "</td>";
								// Assuming you have a column named 'client_number' in your database
								// Assuming you have a column named 'mobile' in your database
								echo "<td>" . (isset($record['email']) ? $record['email'] : '') . "</td>";
								echo "<td>" . (isset($record['mobile']) ? $record['mobile'] : '') . "</td>";
								echo "</tr>";
							}
						} else {
